...
TICK_FLUSH_MS = 100          # coalesce trade ticks to at most 10/sec per client
...
class StreamManager:
    ...
    async def _analysis_loop(self):
        while True:
            try:
                for symbol, interval in self.active_markets():
                    analysis = await asyncio.to_thread(engine.analyze, symbol, interval)
                    payload = {"type": "snapshot", "data": analysis}
                    for c in self.clients:
                        if c.market() == (symbol, interval):
                            c.send(payload)
                # broadcast any newly fired signals to everyone
